...
We need to use CocoaPods, so the first thing is to make sure that `pod` is installed.

```shell
$ brew install cocoapods
# also, make sure your Xcode tools uses the full Xcode installation 
$ sudo xcode-select --switch /Applications/Xcode.app/Contents/Developer 
```

The next step is to initialize cocoapods

```shell
$ cd iosApp/ && pod init && pod install
```

Finally, open the project in xcode `./iosApp/iosApp.xcworkspace` and build it from there.
